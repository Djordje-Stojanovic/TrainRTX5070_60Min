...
0. **Plan:** Read `results.tsv` (what's been tried) AND `ideas.tsv` (experiment idea queue). Pick the top `pending` idea from the queue, set its status to `trying`, and run it. If no pending ideas, identify what is most limiting val_bpb (see Bottleneck-First Rule in `CLAUDE.md`) and add a new idea to `ideas.tsv` before running it. Write your commit message with: `Bottleneck: [what's limiting val_bpb]. Hypothesis: [change] will improve because [reason]. Evidence: [prior experiment / web search / metric]. [idea:UUID]` If you have no evidence, search the web first. **Every 5th experiment**, do a landscape scan across ALL search areas listed in `CLAUDE.md` and add promising NEW ideas to the bottom of `ideas.tsv` (check `results.tsv` first to avoid duplicates). When experiment finishes, set the idea status to `done`.
