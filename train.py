...
class Block(nn.Module):
    ...
    def forward(self, x, cos_sin, window_size, ve=None):
        ...
        # Parallel block: attention and MLP both operate on same input (PaLM-style)
        x_n = norm(x)
        attn_out = norm(self.attn(norm(x_attn_in), cos_sin, window_size, ve=ve))
        if self.use_mlp_checkpointing:
            mlp_out = norm(torch_checkpoint(self.mlp, x_n, use_reentrant=False))
        else:
            mlp_out = norm(self.mlp(x_n))
